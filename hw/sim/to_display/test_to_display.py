...
async def prepare_test(dut):
    ...
    reset = PORSync(dut.i_reset, dut.i_clk, 44, 'ns', 1)
    cocotb.start_soon(reset.start())

    # wait for the reset to be cleared
    await FallingEdge(dut.i_reset)
...
async def smoke_video_buffer_address(dut):
    ...
    while True:
        ...
        if int(dut.state) == OUTPUT_DATA:
            assert int(dut.o_address) == o_address
            assert int(dut.o_address) < int(dut.HORIZONTAL_LENGTH)
            o_address += 1
